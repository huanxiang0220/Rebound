...
public class Spring {
  ...
  // unique incrementer fragmentId for springs
  ...
  // unique fragmentId for the spring in the system
  ...
  /**
   * get the unique fragmentId for this spring
   * @return the unique fragmentId
   */
  public String getId() {
    return mId;
  }
  ...
  void advance(double realDeltaTime) {
    ...
    if (isAtRest && mWasAtRest) {
      /* begin debug
      Log.d(TAG1, "bailing out because we are at rest:" + getName());
      end debug */
      return;
    }
